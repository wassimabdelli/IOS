...
struct MyStaffView: View {
    ...
    private func loadStaff() {
        ...
        // Fallback to user._id if academieId is missing (common for OWNER role)
        let academieId = user.academieId ?? user._id
...
struct AddStaffView: View {
    ...
    private func submitStaff() {
        ...
        // Fallback to user._id if academieId is missing
        let academieId = user.academieId ?? user._id
