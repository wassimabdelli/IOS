...
struct NegotiationScreen: View {
                ...
                if searchText.isEmpty {
                    List(membres, id: \._id) { membre in
                        HStack {
                            ...
                            Spacer()
                            HStack(spacing: 12) {
                                NavigationLink(destination: EditMaillotView(playerId: membre._id, academyId: user._id)) {
                    ...
                    List(searchResults, id: \._id) { joueur in
                        HStack {
                            ...
                            if memberIds.contains(joueur._id) {
                                HStack(spacing: 12) {
                                    NavigationLink(destination: EditMaillotView(playerId: joueur._id, academyId: user._id)) {
                                        Image(systemName: "pencil")
                                            .foregroundColor(.blue)
                                    }
                                    Button(action: {
    ...
    private func loadMembers() {
        isLoading = true
        APIService.getMembresByAcademieCategorie(idAcademie: user._id, categorie: selectedCategorie) { result in
            DispatchQueue.main.async {
                isLoading = false
                switch result {
    ...
    private func addJoueur(joueur: UserModel) {
        APIService.addJoueurToAcademie(idAcademie: user._id, idJoueur: joueur._id, categorie: selectedCategorie) { result in
            DispatchQueue.main.async {
                switch result {
                case .success:
    ...
    private func removeJoueur(joueur: UserModel) {
        APIService.removeJoueurFromAcademie(idAcademie: user._id, idJoueur: joueur._id, categorie: selectedCategorie) { result in
            DispatchQueue.main.async {
                switch result {
                case .success:
    ...
    private func removeMember(_ membre: APIService.TeamMember) {
        APIService.removeJoueurFromAcademie(idAcademie: user._id, idJoueur: membre._id, categorie: selectedCategorie) { result in
            DispatchQueue.main.async {
                switch result {
                case .success:
